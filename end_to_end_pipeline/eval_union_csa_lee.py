...
union-mask CSA output (no refinement), and the union-mask CSA output with
the Lee adaptive speckle filter applied (see lee_filter_experiment.py --all),
over the same scene set. Same three-way comparison structure as
eval_union_csa_threshold.py, swapping the fixed-threshold set for the Lee
filter set.
...
LEE_DIR = os.path.join(BASE, "union_pipeline", "csa_jpg_lee")
