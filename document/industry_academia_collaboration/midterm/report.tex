...
成像目標為由回波 $\mathbf{Y}$ 還原場景反射率 $\mathbf{H}$。傳統成像方法包含 RDA（Range Doppler Algorithm）、CSA（Chirp Scaling Algorithm）、BP（Back-Projection）三類，本研究選擇 \textbf{CSA} 作為基礎成像演算法。
...
為了方便比較，圖 \ref{fig:cross-section} 繪製前對強度值做了正規化處理（僅用於繪圖呈現，非 onboard processing 流程的一部分）：soft-thresholding 的結果加回了 $\lambda$（見 5.2 節），且 CSA 與 CSA + 稀疏精修的輸出皆將其峰值正規化至與原始 SAR 影像的峰值一致。
