...
* Coherent scattering in time domain: $S\circ C+W$
    * Time domain: $C=[c_{m,n}]_{M\times N}$, $c_{m,n}=e^{j\theta_{m,n}}, \theta_{m,n}$ ~ $\mathcal{U}(0,2\pi)$
* Coherent scattering in spatial domain: $\mathcal{T}(S+W)\circ C$
    * $C=[c_{m,n}]_{M\times N}, c_{m,n}$ ~ $\mathcal{E}(1)$
    * $\mathcal{T}$: Imaging algorithm, e.g. RDA or CSA
* Thermal noise (time domain): $W=[w_{m,n}]_{M\times N}, w_{m,n}$ ~ $\mathcal{CN}(0, \sigma)$
